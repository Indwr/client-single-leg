...
//    protected $CI;
    ...
    function is_logged_in() {
        ...
        $user = $ci->user_model->get_single_record('tbl_users',array('user_id' => $ci->session->userdata['user_id']), 'id,user_id,role');
        if(!empty($user)){
            if (isset($ci->session->userdata['user_id']) && $ci->session->userdata['user_id'] == $user['user_id'] && $ci->session->userdata['role'] == 'M') {
                return true;
            } else {
                return false;
            }
...
if (!function_exists('notify_user')) {
    ...
    function notify_user($user_id , $message, $temp_id) {
        $ci = & get_instance();
        $ci->load->model('user_model');
        ...
        if($smsCount['record'] <= 50000){
            $msg = urlencode($message);
            $url = "http://login.smsmedia.org:8381/app/smsapi/index.php?key=35F95958509F3E&campaign=10365&routeid=100867&type=text&contacts=".$user->phone."&senderid=Otpsms&msg=".$msg."&template_id=".$temp_id;
            $ch = curl_init();
