...
//        }
    ...
    public function index() {
        if (is_admin()) {
            ...
            $config['base_url'] = base_url('Jarvis/Withdraw/index');
    ...
    public function Pending() {
        if (is_admin()) {
            ...
            $config['total_rows'] = $this->Main_model->get_sum('tbl_withdraw', $where, 'ifnull(count(id),0) as sum');
            $config['base_url'] = base_url() . 'Jarvis/Withdraw/Pending';
            $config ['uri_segment'] = 4;
            $config['per_page'] = 10;
            $config['attributes'] = array('class' => 'page-link');
            $config['full_tag_open'] = "<ul class='pagination'>";
    ...
    public function income($type = '') {
        if (is_admin()) {
            if($type == 'non_working_income'){
                $table = 'tbl_non_working_wallet';
            }else{
                $table = 'tbl_income_wallet';
            }
            $response['header'] = ucwords(str_replace('_', ' ', $type));
            $config['base_url'] = base_url() . 'Jarvis/Withdraw/income/' . $type;
            $config['total_rows'] = $this->Main_model->get_sum($table, array('type' => $type), 'ifnull(count(id),0) as sum');
            ...
            $response['total_income'] = $this->Main_model->get_sum($table, array('type' => $type), 'ifnull(sum(amount),0) as sum');
            $response['user_incomes'] = $this->Main_model->get_limit_records($table, array('type' => $type), '*', $config['per_page'], $segment);
            $response['segament'] = $segment;
    ...
    public function incomeLedgar($type = '') {
        if (is_admin()) {
            ...
            $config['base_url'] = base_url() . 'Jarvis/Withdraw/incomeLedgar';
    ...
    public function faildPaymentsReturn($id){
        if (is_admin()) {
            ...
            redirect('Jarvis/Withdraw/faildPayments/');
