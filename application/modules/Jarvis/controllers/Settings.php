...
class Settings extends CI_Controller {
    ...
    public function newsDelete($id){
        if (is_admin()) {
            ...
            redirect('Jarvis/Settings/News');
        } else {
            redirect('Jarvis/Management/login');
        }
    }

    public function withdrawonoff($status){
        if (is_admin()) {
            $status = trim(addslashes($status));
            $withdraw_status = $this->Main_model->get_single_record('tbl_admin', array('user_id' => 'admin'), 'withdraw_status');
            if(!empty($withdraw_status)){
                if($status == 0 || $status == 1){
                    $done = $this->Main_model->update('tbl_admin', array('user_id' => 'admin'), array('withdraw_status' => $status));
                    if($done){
                        if($status == 0){$statusMessage = "On";}elseif($status == 1){$statusMessage = "Off";}
                        $this->session->set_flashdata('message', 'Withdraw '.$statusMessage.' Successfully');
                    }else{
                        $this->session->set_flashdata('message', 'Error:: While change request!');
                    }
                }
                redirect('Jarvis/Management/Index');
            }
        } else {
            redirect('Jarvis/Management/login');
        }
    ...
    public function stopUserWithdraw($user_id, $status){
        if (is_admin()) {
            $user_id = trim(addslashes($user_id));
            $status = trim(addslashes($status));
            if(!empty($user_id)){
                $withdraw_status = $this->Main_model->get_single_record('tbl_users', array('user_id' => $user_id), 'withdraw_status,user_id');
                if(!empty($withdraw_status['user_id'])){
                    $this->Main_model->update('tbl_users', array('user_id' => $withdraw_status['user_id']), array('withdraw_status' => $status));
                    redirect('Jarvis/Management/users');
                }
                redirect('Jarvis/Management/login');
            }
        } else {
    ...
    public function payout_summary() {
        if (is_admin()) {
            ...
            $config['base_url'] = base_url('Jarvis/Settings/payout_summary/');
    ...
    public function adminEditProfileOtp($beneficiry_id){
        if (is_admin()) {
            ...
            redirect('Jarvis/Settings/EditUser/'.$beneficiry_id.'');
        }else{
        ...
        if (is_admin()) {
            ...
            redirect('Jarvis/Settings/sendIncome/');
        }else{
            redirect('Dashboard/User/login');
        }
    }
    ...
    public function adminLoginOtp(){
        // if (is_admin()) {
            ...
            redirect('Jarvis/Settings/CreateEpins', $response);
        // }else{
        //     redirect('Jarvis/Management/login');
        // }
    }
    ...
    public function Epins($status) {
        if (is_admin()) {
            ...
            $config['base_url'] = base_url() . 'Jarvis/Settings/Epins/' . $status;
                ...
                if ($this->form_validation->run() != FALSE) {
                    ...
                    if (!empty($user)) {
                        if($data['type'] == 'credit'){
                            ...
                            if ($res == TRUE) {
                                $this->session->set_flashdata('message', 'Amount Credit Successfully');
                            } else {
                            ...
                            $res = $this->Main_model->add('tbl_income_wallet', $IncomeArr);
                            if ($res == TRUE) {
                                $this->session->set_flashdata('message', 'Amount Debit Successfully');
                            } else {
                                $this->session->set_flashdata('message', 'Error While Transferring Amount  Please Try Again ...');
                            }
                        }
                ...
                if ($this->form_validation->run() != FALSE) {
                    ...
                    if($oldPass['password'] == $data['old_password']){
                        if($data['new_password'] == $data['confirm_password']){
                            ...
                            if ($res == TRUE) {
                                $this->session->set_flashdata('message', 'Password updated Successfully');
                                redirect('Jarvis/Settings/updatePassword');
                            } else {
    ...
    public function sendSms()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $this->form_validation->set_rules('contacts', 'Phone No.', 'trim|required|xss_clean');
            if ($this->form_validation->run() != FALSE) {
                $msg = urlencode($data['message']);
                $url = "http://login.smsmedia.org:8381/app/smsapi/index.php?key=35F95958509F3E&campaign=10365&routeid=100867&type=text&contacts=".$data['contacts']."&senderid=Otpsms&msg=".$msg;
                $ch = curl_init();
                ...
                $this->Main_model->add('tbl_sms_counter', $sms_data);
                if($sms_data){
                     $this->session->set_flashdata('message', 'Message send Successfully!');
                }else{
                     $this->session->set_flashdata('message', 'Something wrong in query!');
                }
            }
        }
        $this->load->view('sendSms', $response);   
    }
}
