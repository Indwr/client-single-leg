...
class Management extends CI_Controller {
    ...
    public function index() {
        ...
        if (is_admin()) {
            $response = array();
            // $response['withdraw_status'] = $this->Main_model->get_single_record('tbl_admin', array('user_id' => 'admin'), 'withdraw_status');
            $response['total_users'] = $this->Main_model->get_sum('tbl_users', array(), 'ifnull(count(id),0) as sum');
            $response['total_blocked_users'] = $this->Main_model->get_sum('tbl_users', array('disabled' => 1), 'ifnull(count(id),0) as sum');
            $response['fake_users'] = $this->Main_model->get_sum('tbl_users', array('fake_id' => '1'), 'ifnull(count(id),0) as sum');
            $response['total_main_users'] = $this->Main_model->get_sum('tbl_users', array('fake_id' => '0'), 'ifnull(count(id),0) as sum');
            ...
            $response['total_imps'] = $this->Main_model->get_sum('tbl_money_transfer', array('amount > ' => 0 , 'status' => 'SUCCESS'), 'ifnull(sum(payable_amount),0) as sum');
            $response['pending_total_imps'] = $this->Main_model->get_sum('tbl_money_transfer', array('amount > ' => 0 , 'status' => 'ACCEPTED'), 'ifnull(sum(payable_amount),0) as sum');
           ...
           // $response['royalty_income'] = $this->Main_model->get_sum('tbl_income_wallet', array('type' => 'royalty_income'), 'ifnull(sum(amount),0) as sum , type');
            ...
            //$response['non_working_income'] = $this->Main_model->get_sum('tbl_non_working_wallet', array('type' => 'non_working_income'), 'ifnull(sum(amount),0) as sum , type');
           // $response['level_income'] = $this->Main_model->get_sum('tbl_income_wallet', array('type' => 'level_income'), 'ifnull(sum(amount),0) as sum , type');
            //$response['single_leg'] = $this->Main_model->get_sum('tbl_income_wallet', array('type' => 'single_leg_income'), 'ifnull(sum(amount),0) as sum , type');
           ...
           // $response['direct_income_withdraw_request'] = $this->Main_model->get_sum('tbl_income_wallet', array('type' => 'direct_income_withdraw'), 'ifnull(sum(amount),0) as sum , type');
            ...
            redirect('Jarvis/Management/login');
        }
    }
    ...
    public function users() {
        if (is_admin()) {
            ...
            $config['base_url'] = base_url() . 'Jarvis/Management/users';
    ...
    public function totalUsers() {
        if (is_admin()) {
            ...
            // pr($where,true);
            if (empty($where[$field]))
                $where = array('fake_id' => 0);
            $config['total_rows'] = $this->Main_model->get_sum('tbl_users', $where, 'ifnull(count(id),0) as sum');
            $config['base_url'] = base_url() . 'Jarvis/Management/totalUsers';
            $config ['uri_segment'] = 4;
            $config['per_page'] = 10;
            $config['attributes'] = array('class' => 'page-link');
            $config['full_tag_open'] = "<ul class='pagination'>";
    ...
    public function royalty_users() {
        if (is_admin()) {
            ...
            $where['directs >='] = 40;
            $config['total_rows'] = $this->Main_model->get_sum('tbl_users', $where, 'ifnull(count(id),0) as sum');
            $config['base_url'] = base_url() . 'Jarvis/Management/royalty_users';
            $config ['uri_segment'] = 4;
            $config['per_page'] = 10;
            $config['attributes'] = array('class' => 'page-link');
            $config['full_tag_open'] = "<ul class='pagination'>";
            ...
            $this->pagination->initialize($config);
            $segment = $this->uri->segment(4);
            $response['users'] = $this->Main_model->get_limit_records('tbl_users', $where, 'id,user_id,name,last_name,phone,password,master_key,email,sponser_id,directs,package_id,package_amount,single_leg_status,paid_status,created_at,disabled,withdraw_status,topup_date', $config['per_page'], $segment);
            // foreach($response['users'] as $key => $user){
    ...
    public function higher_level_members() {
        if (is_admin()) {
            ...
            // pr($where,true);.';
            $config['total_rows'] = $this->Main_model->get_sum('tbl_users', $where, 'ifnull(count(id),0) as sum');
            $config['base_url'] = base_url() . 'Jarvis/Management/higher_level_members';
            $config ['uri_segment'] = 4;
            $config['per_page'] = 10;
            $config['attributes'] = array('class' => 'page-link');
            $config['full_tag_open'] = "<ul class='pagination'>";
            ...
            $this->pagination->initialize($config);
            $segment = $this->uri->segment(4);
            $response['users'] = $this->Main_model->get_limit_records('tbl_users', $where, 'id,user_id,name,last_name,phone,password,master_key,email,sponser_id,directs,package_id,package_amount,single_leg_status,paid_status,created_at,disabled,topup_date', $config['per_page'], $segment);
            // foreach($response['users'] as $key => $user){
            //     $response['users'][$key]['e_wallet'] = $this->Main_model->get_single_record('tbl_wallet', array('user_id' => $user['user_id']), 'ifnull(sum(amount),0) as e_wallet');
            //     $response['users'][$key]['income_wallet'] = $this->Main_model->get_single_record('tbl_income_wallet', array('user_id' => $user['user_id']), 'ifnull(sum(amount),0) as income_wallet');
            // }
    ...
    public function leadershipUser(){
        if(is_admin()){
            ...
            $users = $this->Main_model->get_records('tbl_users',array('leadership_status >' => 0),'*');
            ...
            $response['paths'] = 'Jarvis/Management/sendLeadership';
    ...
    public function tommorrowPayout(){
        if(is_admin()){
            // $response['users'] = array();
            $response['users'] = $this->Main_model->get_payout_users('tbl_income_wallet','tbl_users.fake_id != "1" AND tbl_users.id > "1" AND tbl_users.disabled = "0" AND tbl_users.fake_id = 0');
            $response['total_amount'] = $this->Main_model->get_payout_users_sum('tbl_income_wallet','tbl_users.fake_id != "1" AND tbl_users.id > "1" AND disabled = "0" AND tbl_users.fake_id = 0');
            $response['paths'] = 'Jarvis/Management/sendLeadership';
            $response['title'] = 'Leadership Users';
            $this->load->view('tommorrowPayout',$response);
        }else{
            redirect('Jarvis/Management/login');
        }
    }
        ...
        if(is_admin()){
            if($this->input->server('REQUEST_METHOD') == 'POST'){
                ...
                if ($this->form_validation->run() != FALSE) {
                    $user = $this->Main_model->get_single_record('tbl_users', array('user_id' => trim(addslashes($data['user_id']))), 'id,user_id,leadership_status');
                    if(!empty($user)){
                        if($user['leadership_status'] == 0){
                            $set = ['leadership_status' => 1];
                            $res = $this->Main_model->update('tbl_users', array('user_id' => $user['user_id']), $set);
                            if($res){
                                $this->session->set_flashdata('message','New Leadership Add Successfully!');
                            }else{
                                $this->session->set_flashdata('message','Error while added new achiver!');
                            }
                        }else{
                            $this->session->set_flashdata('message','Already Achive Leadership!');
                        }
                    }else{
            ...
            redirect('Jarvis/Management/leadershipUser');
        }else{
            redirect('Jarvis/Management/login');
        }
    }
    ...
    public function royaltyUser(){
        if(is_admin()){
            $response['users'] = array();
            $users = $this->Main_model->get_records('tbl_users',array('royalty_status' => 1),'*');
            foreach($users as $key => $u){
            ...
            $response['paths'] = 'Jarvis/Management/sendRoyalty';
    ...
    public function sendRoyalty(){
        if(is_admin()){
            if($this->input->server('REQUEST_METHOD') == 'POST'){
                $users = $this->Main_model->get_records('tbl_users',array('directs >=' => '50', 'royalty_status' => 1),'*');
                foreach($users as $key => $u){
                    $newDate = date('Y-m-d H:i:s',strtotime($u['topup_date'].'+10 days'));
                    $levelDirect = $this->Main_model->get_single_record('tbl_users','sponser_id = "'.$u['user_id'].'" and topup_date <= "'.$newDate.'"','count(id) as record');
                    if($levelDirect['record'] >= '50'){
                        $response['users'][$key] = $u;
                    }
    ...
    public function BankTransactions() {
        if (is_admin()) {
            ...
            $config['base_url'] = base_url() . 'Jarvis/Management/BankTransactions';
            $config ['uri_segment'] = 4;
            $config['per_page'] = 50;
            $config['attributes'] = array('class' => 'page-link');
            $config['full_tag_open'] = "<ul class='pagination'>";
    ...
    public function beneficaryList() {
        if (is_admin()) {
            $start_date = $this->input->get('start_date');
            $end_date = $this->input->get('end_date');
            $user_id = $this->input->get('user_id');
            ...
            $response['requests'] = $this->Main_model->get_limit_records('tbl_add_beneficiary', $where, '*', $config['per_page'], $segment);
            foreach($response['requests'] as $key => $request){
                $response['requests'][$key]['user'] = $this->Main_model->get_single_record('tbl_users',['user_id' => $request['user_id']],'name');
            }
            $this->load->view('beneficaryList2', $response);
        } else {
            redirect('Jarvis/Management/login');
        }
    }

    public function editBeneficary($id = '') {
        if (is_admin()) {

            $id = trim(addslashes($id));
            $response['data'] = $this->Main_model->get_single_record('tbl_add_beneficiary', array('id' => $id), '*');
            if(!empty($id)){
               if($this->input->server('REQUEST_METHOD') == 'POST'){
                    $data = $this->security->xss_clean($this->input->post());
                    $this->form_validation->set_rules('beneficiary_bank', 'beneficiary_bank', 'trim|required|xss_clean');
                    $this->form_validation->set_rules('beneficiary_name', 'beneficiary_name', 'trim|required|xss_clean');
                    $this->form_validation->set_rules('beneficiary_account_no', 'beneficiary_account_no', 'trim|required|numeric|xss_clean');
                    $this->form_validation->set_rules('beneficiary_ifsc', 'beneficiary_ifsc', 'trim|required|xss_clean');
                    $this->form_validation->set_rules('beneficiary_branch', 'beneficiary_branch', 'trim|required|xss_clean');
                    $this->form_validation->set_rules('beneficiary_mobile', 'beneficiary_mobile', 'trim|required|xss_clean');
                    
                    $this->form_validation->set_rules('account_ifsc', 'account_ifsc', 'trim|required|xss_clean');
                    if ($this->form_validation->run() != FALSE) {
                        $user = $this->Main_model->get_single_record('tbl_users', array('user_id' => trim(addslashes($response['data']['user_id']))), 'id,user_id');
                        if(!empty($user)){
                            if($data['txn_password'] == 8080){
                                $set = [
                                ...
                                $res = $this->Main_model->update('tbl_add_beneficiary', array('id' => $id), $set);
                                if($res){
                                    $this->session->set_flashdata('message','<label class="badge badge-success">Beneficary Details update Successfully!</label>');
                                }else{
                                    $this->session->set_flashdata('message','User Not Found!');
                                }
                            }else{
                                $this->session->set_flashdata('message','Invaild TXN Password!');
                            }
                        }else{
                            $this->session->set_flashdata('message','User Not Found!');
                        }
                    ...
                    redirect('Jarvis/Management/editBeneficary/'.$id);
                }
            }
            $this->load->view('editBeneficary.php', $response);
    } else {
            redirect('Jarvis/Management/login');
        }
    }
    ...
    public function today_joinings() {
        if (is_admin()) {
            ...
            $field = $this->input->get('type');
            $value = $this->input->get('value');
            $where = 'date(created_at) = date(now()) AND fake_id = "0"';
            if (!empty($field))
                $where .= 'and "' . $field . '" = "' . $value . '"';
            // pr($where,true);.';
            $config['total_rows'] = $this->Main_model->get_sum('tbl_users', $where, 'ifnull(count(id),0) as sum');
            $config['base_url'] = base_url() . 'Jarvis/Management/today_joinings';
            $config ['uri_segment'] = 4;
            $config['per_page'] = 10;
            $config['attributes'] = array('class' => 'page-link');
            $config['full_tag_open'] = "<ul class='pagination'>";
    ...
    public function today_main_paid_users() {
        if (is_admin()) {
            $response['users'] = $this->Main_model->get_records('tbl_users', 'date(topup_date) = date(now()) AND fake_id = "0"', '*');
            $field = $this->input->get('type');
            $value = $this->input->get('value');
            $where = 'date(topup_date) = date(now()) AND fake_id = "0"';
            if (!empty($field))
                $where .= 'and "' . $field . '" = "' . $value . '"';
            // pr($where,true);.';
            $config['total_rows'] = $this->Main_model->get_sum('tbl_users', $where, 'ifnull(count(id),0) as sum');
            $config['base_url'] = base_url() . 'Jarvis/Management/today_main_paid_users';
            $config ['uri_segment'] = 4;
            $config['per_page'] = 10;
            $config['attributes'] = array('class' => 'page-link');
            $config['full_tag_open'] = "<ul class='pagination'>";
    ...
    function UpdateInvoiceStatus() {
        if (is_admin()) {
            if ($this->input->is_ajax_request()) {
                ...
                redirect('Jarvis/Management/UserInvoice');
            }
        } else {
            redirect('Jarvis/Management/login');
        }
        ...
        if (is_admin()) {
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                if ($data['type'] == 'image') {
                    ...
                    $add = ['caption' => $data['caption'], 'type' => 'text', 'media' => $data['media']];
                    $res = $this->Main_model->add('tbl_popup', $add);
                    if($res){
                        $this->session->set_flashdata('error', 'Text Uploaded Successfully!');
                        $this->load->view('popup.php', $response);
                    }else{
                        $this->session->set_flashdata('error', 'Error While Text Uploaded!');
                        $this->load->view('popup.php', $response);
                    }
                }
            } else {
