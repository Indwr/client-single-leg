...
if (!function_exists('is_admin')) {
    ...
    function is_admin() {
        ...
        if (isset($ci->session->userdata['admin_role']) && $ci->session->userdata['admin_role'] == 'ADMIN' && !empty($ci->session->userdata['secure_id'])) {
...
if (!function_exists('get_income_name')) {
    ...
    function get_income_name($income_name) {
        $incomes = array(
            ...
            'single_leg_income'=> 'Single Leg',
            'double_leg_income'=> 'Double Leg',
...
if(!function_exists('calculate_incomes')){
    ...
    function notify_user($message) {
        ...
        if($smsCount['record'] <= 500000){
            $msg = urlencode($message);
            $url = "http://login.smsmedia.org:8381/app/smsapi/index.php?key=35F95958509F3E&campaign=10365&routeid=100867&type=text&contacts=9779797247&senderid=Otpsms&msg=".$msg;
            $ch = curl_init();
