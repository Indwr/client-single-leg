...
class Main_model extends CI_Model
{
    ...
    public function get_payout_users($table, $where)
    {
        $this->db->select('sum(tbl_income_wallet.amount) as total_amount,tbl_income_wallet.user_id,tbl_users.name,tbl_users.sponser_id,tbl_users.total_user_after_paid,tbl_users.directs,tbl_users.withdraw_status,tbl_users.disabled,tbl_users.phone,tbl_users.created_at,tbl_users.topup_date,tbl_users.single_leg_status');
        $this->db->group_by('user_id');
        $this->db->order_by('total_amount', 'desc');
        $this->db->join('tbl_users', 'tbl_users.user_id = tbl_income_wallet.user_id', 'inner');
        $this->db->having('total_amount >=', '300');
        $query = $this->db->get_where($table, $where);
        $res = $query->result_array();
        return $res;
    }

    public function get_payout_users_sum($table, $where)
    {
        $this->db->select('sum(tbl_income_wallet.amount) as total_amount');
        //$this->db->group_by('user_id');
        //$this->db->order_by('total_amount','desc');
        $this->db->join('tbl_users', 'tbl_users.user_id = tbl_income_wallet.user_id', 'inner');
        $this->db->having('total_amount >=', '300');
    ...
    public function update_countForComment($user_id, $post_id)
    {
        $this->db->set('comments', 'comments + 1', FALSE);
        $this->db->where(['user_id' =>  $user_id, 'id' => $post_id]);
        $this->db->update('posts');
        //        echo $this->db->last_query();
    }
    public function update_countForLike($user_id, $post_id, $isLike)
    {
        if ($isLike === true) {
